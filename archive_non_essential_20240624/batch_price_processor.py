...
from daily_run.common_imports import *
from daily_run.database import DatabaseManager
from daily_run.error_handler import ErrorHandler, ErrorSeverity
from daily_run.monitoring import SystemMonitor
...
class BatchPriceProcessor:
    ...
    def __init__(self, db: DatabaseManager, max_batch_size: int = 100, 
        ...
        # Initialize Polygon.io service if available
        try:
            from daily_run.polygon_service import PolygonService
            self.polygon_service = PolygonService()
            logger.info("Polygon.io service initialized for batch processing")
        ...
        self.service_priority = [
            ...
            ('Polygon.io', self._get_polygon_batch_prices),
    ...
    def process_batch_prices(self, tickers: List[str]) -> Dict[str, Dict]:
        ...
        try:
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        self.error_handler.handle_error(
                            e, {'operation': f'batch_price_processing_batch_{batch_num}', 'severity': 'HIGH'}
                        )
            ...
            self.error_handler.handle_error(
                e, {'operation': 'batch_price_processing', 'severity': 'CRITICAL'}
            )
            return {}
    ...
    def _get_finnhub_batch_prices(self, tickers: List[str]) -> Dict[str, Dict]:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=min(5, len(tickers))) as executor:
                future_to_ticker = {
                    executor.submit(self.finnhub_service.get_data, ticker): ticker 
    ...
    def _get_polygon_batch_prices(self, tickers: List[str]) -> Dict[str, Dict]:
        """Get batch prices from Polygon.io"""
        try:
            if self.polygon_service:
                return self.polygon_service.get_batch_prices(tickers)
            else:
                logger.warning("Polygon.io service not available")
                return {}
                
        except Exception as e:
            logger.error(f"Polygon.io batch request failed: {e}")
            return {}
    ...
    def store_daily_prices(self, price_data: Dict[str, Dict]):
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, {'operation': 'store_daily_prices', 'severity': 'HIGH'}
            )
