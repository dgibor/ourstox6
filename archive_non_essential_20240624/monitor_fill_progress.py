...
def monitor_fill_progress():
    ...
    print("📊 MONITORING FILL ALL COMPANY FUNDAMENTALS PROGRESS")
    print("=" * 60)
    ...
    any_query = "SELECT COUNT(*) FROM company_fundamentals"
    ...
    sample_query = """
    SELECT ticker, revenue, net_income, market_cap, last_updated
    FROM company_fundamentals 
    WHERE revenue IS NOT NULL 
    AND net_income IS NOT NULL 
    AND market_cap IS NOT NULL
    ...
    if sample_data:
        ...
        for row in sample_data:
            ticker, revenue, net_income, market_cap, last_update = row
