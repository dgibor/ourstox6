#!/usr/bin/env python3
"""
Prioritized Fundamental Data Filler
Processes companies by market cap (largest first) using FMP batch API calls
"""
...
class PrioritizedFundamentalFiller:
    """Prioritized fundamental data filler - largest market cap first"""
    ...
    def __init__(self):
        ...
        logger.info("Prioritized Fundamental Data Filler initialized")
    ...
    def get_tickers_by_market_cap(self) -> List[Dict]:
        """Get all tickers ordered by market cap (largest first)"""
        try:
            # Get all tickers with market cap, ordered by market cap descending
            query = """
            ...
            logger.info(f"Found {len(tickers)} tickers ordered by market cap")
            
            # Log top 10 by market cap
            if tickers:
                logger.info("Top 10 companies by market cap:")
                for i, ticker_info in enumerate(tickers[:10]):
            ...
            logger.error(f"Error getting tickers by market cap: {e}")
            return []
    ...
    def process_by_market_cap(self, limit: int = None) -> Dict[str, Any]:
        """Process tickers by market cap (largest first)"""
        logger.info("🚀 Starting prioritized fundamental data filling process")
        ...
        # Get tickers ordered by market cap
        ticker_info_list = self.get_tickers_by_market_cap()
        if limit:
        ...
        logger.info(f"Processing {self.total_tickers} tickers by market cap priority")
        ...
        logger.info("✅ Prioritized fundamental data filling completed")
...
def main():
    """Main function to run the prioritized fundamental data filling process"""
    print("🚀 Starting Prioritized Fundamental Data Filling Process")
    print("=" * 60)
    print("Processing companies by market cap (largest first)")
    print()
    
    try:
        filler = PrioritizedFundamentalFiller()
        
        # Ask user for processing limit
        print("Choose processing scope:")
        print("  1. Test with top 10 companies")
        print("  2. Test with top 25 companies") 
        print("  3. Test with top 50 companies")
        print("  4. Test with top 100 companies")
        print("  5. Process all companies (use with caution)")
        ...
        try:
            ...
            if choice == '1':
                ...
                print("Invalid choice, using top 25 companies as default")
                limit = 25
            
            # Process tickers by market cap
            results = filler.process_by_market_cap(limit=limit)
            ...
            print("\n✅ Prioritized fundamental data filling completed")
