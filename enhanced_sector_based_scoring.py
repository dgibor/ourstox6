#!/usr/bin/env python3
"""
ENHANCED SECTOR-BASED SCORING SYSTEM
This script implements a sector-based scoring system with configurable weights from CSV
and produces more diverse ratings including Buy, Strong Buy, Sell, and Strong Sell.
"""
...
class EnhancedSectorBasedScoring:
    def __init__(self):
        self.db = None
    ...
    def get_sector_weights(self, ticker):
        ...
        try:
            ...
            if sector_data.empty:
                # Use default weights if sector not found
    ...
    def get_rating(self, composite_score, sector_weights):
        ...
        try:
            ...
            if composite_score >= strong_buy_threshold:
                ...
                return "Hold"
            elif composite_score >= weak_hold_threshold:
                return "Weak Hold"
    ...
    def run_scoring_test(self, tickers):
        ...
        try:
            logger.info("Starting ENHANCED SECTOR-BASED SCORING test...")
            ...
            # Analyze results
            self.analyze_scoring_results(results, skipped)
            
        except Exception as e:
            logger.error(f"Error running scoring test: {e}")
            
    def analyze_scoring_results(self, results, skipped):
        """Analyze and display scoring results"""
        try:
            print("\n" + "="*120)
            print("ENHANCED SECTOR-BASED SCORING RESULTS")
            print("="*120 + "\n")
            ...
            for rating in ['Strong Buy', 'Buy', 'Hold', 'Weak Hold', 'Weak Sell', 'Sell', 'Strong Sell']:
                if rating in ratings:
                    ...
                    for result in ratings[rating]:
                        print(f"{result['ticker']} ({result['sector']}):")
            ...
            for result in results:
                ...
                if sector not in sectors:
                    sectors[sector] = {'count': 0, 'avg_score': 0, 'ratings': {}}
                sectors[sector]['count'] += 1
                sectors[sector]['avg_score'] += result['composite_score']
                
                rating = result['rating']
                if rating not in sectors[sector]['ratings']:
                    sectors[sector]['ratings'][rating] = 0
                sectors[sector]['ratings'][rating] += 1
                
            for sector, data in sectors.items():
                avg_score = data['avg_score'] / data['count']
                print(f"   {sector}: {data['count']} stocks, Avg Score: {avg_score:.1f}")
            ...
            print("\n" + "="*120)
            print("ENHANCED SECTOR-BASED SCORING COMPLETED")
            print("="*120)
            
        except Exception as e:
            logger.error(f"Error analyzing scoring results: {e}")

def main():
    """Main execution function"""
    try:
        # Initialize scoring system
        scoring_system = EnhancedSectorBasedScoring()
