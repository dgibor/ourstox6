#!/usr/bin/env python3
"""
FINAL DECISIVE Stock Scoring System
Focuses on stocks with correct price data and provides realistic Buy/Sell ratings
Aligns with current market sentiment and AI analyst recommendations
"""
...
class FinalDecisiveScorer:
    """Final decisive stock scorer with realistic market alignment"""
    ...
    def calculate_fundamental_health(self, fundamental: Dict) -> float:
        """Calculate fundamental health score (0-100) - REALISTIC DECISIVE"""
        try:
            score = 0
            factors = 0
            
            # Market Cap - Realistic scoring
            ...
            if fundamental.get('free_cash_flow') and fundamental['free_cash_flow'] > 0:
                score += 18  # Strong cash flow
                factors += 1
            
            final_score = score / factors if factors > 0 else 50.0
            
            # Realistic scoring - wider ranges for decisiveness
            if final_score >= 85:
    ...
    def calculate_technical_health(self, technical: Dict, price_history: List) -> float:
        """Calculate technical health score (0-100) - REALISTIC DECISIVE"""
        try:
            score = 0
            factors = 0
            
            # Price momentum - Realistic
    ...
    def calculate_vwap_sr_score(self, technical: Dict, ticker: str) -> Tuple[float, Dict]:
        """Calculate VWAP & Support/Resistance score (0-100) - REALISTIC DECISIVE"""
        try:
            current_price = technical['close'] / 100.0
            ...
            # VWAP Analysis (40% weight) - Realistic
            vwap_score = 0
            if vwap > 0:
            ...
            # Support Analysis (30% weight) - Realistic
            support_score = 0
                ...
                if (self.is_price_data_valid(support_1, ticker) and 
                    ...
                    if supports:
                        ...
                        if distance_to_support <= 0.02:  # Within 2% of support
                            ...
                            support_score = 45  # No support nearby
                else:
                    support_score = 70  # Neutral if support data is corrupted
            
            # Resistance Analysis (30% weight) - Realistic
            resistance_score = 0
                ...
                if (self.is_price_data_valid(resistance_1, ticker) and 
                    ...
                    if resistances:
                        ...
                        if distance_to_resistance <= 0.02:  # Within 2% of resistance
                            resistance_score = 15   # Very close to resistance (very bearish)
                        elif distance_to_resistance <= 0.05:  # Within 5% of resistance
                            resistance_score = 25   # Close to resistance (bearish)
                        elif distance_to_resistance <= 0.10:  # Within 10% of resistance
                            resistance_score = 40   # Near resistance
                        elif distance_to_resistance <= 0.15:  # Within 15% of resistance
                            resistance_score = 55   # Moderate distance
                        else:  # Far from resistance (bullish)
                            resistance_score = 90   # Clear path to resistance
                else:
    ...
    def get_rating(self, score: float) -> str:
        """Get rating based on score - FINAL DECISIVE THRESHOLDS"""
        if score >= 85:
            return "Strong Buy"
        elif score >= 75:
            return "Buy"
    ...
    def test_scoring_system(self, test_tickers: List[str] = None):
        """Test the final decisive scoring system"""
        if not test_tickers:
        ...
        # Display results
        print("=" * 100)
        print("FINAL DECISIVE STOCK SCORING SYSTEM RESULTS - VALID DATA STOCKS ONLY")
        print("=" * 100)
        ...
        print(f"\n🔍 KEY IMPROVEMENTS:")
        print(f"   • Data quality validation - only stocks with valid prices")
        print(f"   • Realistic scoring thresholds for market alignment")
        print(f"   • Heavy VWAP & S/R weight (40% of total score)")
        print(f"   • Automatic filtering of corrupted data")
        
        print(f"\n{'='*100}")
        print("FINAL DECISIVE SCORING COMPLETED")
        print(f"{'='*100}")

def main():
    """Main function to test final decisive scoring"""
    scorer = FinalDecisiveScorer()
    ...
    try:
        logger.info("Starting FINAL DECISIVE stock scoring test...")
