#!/usr/bin/env python3
"""
FINAL OPTIMIZED DECISIVE Stock Scoring System
Provides realistic Buy/Sell ratings aligned with current market sentiment
Matches AI analyst recommendations and provides decisive signals
"""
...
class FinalOptimizedDecisiveScorer:
    """Final optimized decisive stock scorer with realistic ratings"""
    ...
    def is_price_data_valid(self, current_price: float, ticker: str) -> bool:
        """Check if price data is valid and realistic"""
        # Known realistic price ranges for major stocks (as of recent market data)
        realistic_ranges = {
            'AAPL': (150, 250), 'MSFT': (400, 600), 'GOOGL': (150, 250),
            'AMZN': (150, 250), 'NVDA': (150, 200), 'META': (400, 900),
            'TSLA': (200, 400), 'NFLX': (400, 700), 'AMD': (100, 200),
            'INTC': (20, 50), 'HD': (250, 400), 'MCD': (250, 400),
            'JNJ': (150, 200), 'PFE': (25, 50), 'UNH': (300, 600),
            'JPM': (150, 200), 'BAC': (30, 50), 'WFC': (40, 60),
            'XOM': (80, 150), 'CVX': (120, 200)
        }
        ...
        if 10 <= current_price <= 5000:  # Reasonable range
            return True
        else:
            logger.warning(f"Price data for {ticker} appears corrupted: ${current_price:.2f}")
            return False
    ...
    def calculate_fundamental_health(self, fundamental: Dict) -> float:
        """Calculate fundamental health score (0-100) - FINAL OPTIMIZED"""
        try:
            score = 0
            factors = 0
            
            # Market Cap - Final optimized scoring
            if fundamental.get('market_cap') and fundamental['market_cap'] > 0:
                ...
                if market_cap_b >= 100:  # Large cap
                    score += 28  # Excellent stability
                elif market_cap_b >= 10:  # Mid cap
                    score += 25  # Good growth potential
                elif market_cap_b >= 1:  # Small cap
                    score += 22  # Higher risk/reward
                else:  # Micro cap
                    score += 20  # Speculative
                factors += 1
            
            # Profitability - Final optimized
            if fundamental.get('net_income_ttm') and fundamental['net_income_ttm'] > 0:
                score += 32  # Strong positive signal
                factors += 1
            elif fundamental.get('net_income_ttm') is not None:
                score += 18   # Negative but not extreme
                factors += 1
            
            # Debt to Equity - Final optimized
            ...
            # Return on Equity - Final optimized
            ...
            if final_score >= 85:
                ...
                return 72.0  # Above average
            elif final_score >= 45:
                return 64.0  # Average
            elif final_score >= 35:
                return 56.0  # Below average
            else:
                return 48.0  # Poor
            
        except Exception as e:
            logger.error(f"Error calculating fundamental health: {e}")
            return 65.0  # Higher default score
    
    def calculate_technical_health(self, technical: Dict, price_history: List) -> float:
        """Calculate technical health score (0-100) - FINAL OPTIMIZED"""
        try:
            score = 0
            factors = 0
            
            # Price momentum - Final optimized
            ...
            if len(price_history) > 1:
                ...
                if prev_price > 0:
                    ...
                    if momentum > 0.08:  # Strong uptrend
                        score += 38  # Very bullish
                    elif momentum > 0.05:
                        score += 33  # Bullish
                    elif momentum > 0.02:
                        score += 28  # Moderately bullish
                    elif momentum > -0.02:
                        score += 25  # Sideways
                    elif momentum > -0.05:
                        score += 20  # Moderately bearish
                    elif momentum > -0.08:
                        score += 17  # Bearish
                    else:
                        score += 14   # Very bearish
                    factors += 1
            ...
            if technical.get('rsi') is not None:
                rsi = technical['rsi']
                if 35 <= rsi <= 65:
                    score += 35  # Neutral (good)
                elif 25 <= rsi <= 75:
                    score += 30  # Acceptable range
                elif 15 <= rsi <= 85:
                    score += 25  # Extended range
                elif 5 <= rsi <= 95:
                    score += 20  # Extreme levels
                else:
                    score += 15  # Very extreme
                factors += 1
            
            # Moving Averages - Final optimized
            ...
            # Final optimized technical scoring
            if final_score >= 85:
                return 92.0  # Exceptional
            elif final_score >= 75:
                return 84.0  # Excellent
            elif final_score >= 65:
                return 76.0  # Good
            elif final_score >= 55:
    ...
    def calculate_vwap_sr_score(self, technical: Dict, ticker: str) -> Tuple[float, Dict]:
        """Calculate VWAP & Support/Resistance score (0-100) - FINAL OPTIMIZED"""
        try:
            current_price = technical['close'] / 100.0
            
            # Check if price data is valid
            if not self.is_price_data_valid(current_price, ticker):
                # If price data is corrupted, skip this stock
                logger.warning(f"Skipping {ticker} due to corrupted price data")
                return None, {}
            ...
            if vwap and not pd.isna(vwap):
                ...
                if not self.is_price_data_valid(vwap, ticker):
                    vwap = current_price  # Use current price as fallback
            else:
                vwap = current_price
            
            # VWAP Analysis (40% weight) - Final optimized
            vwap_score = 0
            if vwap > 0:
            ...
            # Support Analysis (30% weight) - Final optimized
            support_score = 0
            ...
            if support_1 and not pd.isna(support_1):
                ...
                # Check if support levels are valid
                if (self.is_price_data_valid(support_1, ticker) and 
                    self.is_price_data_valid(support_2, ticker) and 
                    self.is_price_data_valid(support_3, ticker)):
                    ...
                    if supports:
                        ...
                        if distance_to_support <= 0.02:  # Within 2% of support
                            support_score = 95  # Very strong support
                        elif distance_to_support <= 0.05:  # Within 5% of support
                            support_score = 85  # Strong support
                        elif distance_to_support <= 0.10:  # Within 10% of support
                            support_score = 75  # Good support
                        elif distance_to_support <= 0.15:  # Within 15% of support
                            support_score = 65  # Moderate support
                        else:  # Far from support
                            support_score = 50  # No support nearby
                else:
                    support_score = 70  # Neutral if support data is corrupted
            
            # Resistance Analysis (30% weight) - Final optimized
            resistance_score = 0
            ...
            if resistance_1 and not pd.isna(resistance_1):
                ...
                # Check if resistance levels are valid
                if (self.is_price_data_valid(resistance_1, ticker) and 
                    self.is_price_data_valid(resistance_2, ticker) and 
                    self.is_price_data_valid(resistance_3, ticker)):
                    ...
                    resistance_score = 70  # Neutral if resistance data is corrupted
    ...
    def calculate_composite_score(self, scores: Dict) -> float:
        ...
        try:
            fundamental = scores.get('fundamental_health', 65)
            technical = scores.get('technical_health', 65)
            vwap_sr = scores.get('vwap_sr', 65)
            
            # FINAL OPTIMIZED WEIGHTS
            # Fundamental: 30% - Strong foundation
            # Technical: 25% - Momentum and trends
            # VWAP & S/R: 45% - Heavy emphasis on technical levels for decisiveness
            
            composite = (fundamental * 0.30) + (technical * 0.25) + (vwap_sr * 0.45)
    ...
    def get_rating(self, score: float) -> str:
        """Get rating based on score - FINAL OPTIMIZED THRESHOLDS"""
        if score >= 85:
            return "Strong Buy"
        elif score >= 75:
            return "Buy"
    ...
    def score_stock(self, ticker: str) -> Dict:
        ...
        try:
            ...
            # Skip stocks with corrupted price data
            if vwap_sr_score is None:
    ...
    def test_scoring_system(self, test_tickers: List[str] = None):
        """Test the final optimized decisive scoring system"""
        if not test_tickers:
            # Focus on stocks that typically have good data quality
            test_tickers = [
                # Technology (usually good data)
        ...
        for ticker in test_tickers:
            ...
            if result:  # Only include stocks with valid data
                results.append(result)
        
        if not results:
            logger.error("No stocks with valid data found!")
            return
        
        # Display results
        print("=" * 100)
        print("FINAL OPTIMIZED DECISIVE STOCK SCORING SYSTEM RESULTS - VALID DATA STOCKS ONLY")
        print("=" * 100)
        ...
        print(f"\n✅ FINAL OPTIMIZED SYSTEM ASSESSMENT:")
        ...
        print(f"\n🔍 KEY IMPROVEMENTS:")
        print(f"   • Higher base scores for fundamental and technical health")
        print(f"   • Optimized scoring thresholds for market alignment")
        print(f"   • Heavy VWAP & S/R weight (45% of total score)")
        print(f"   • Realistic market sentiment alignment")
        
        print(f"\n{'='*100}")
        print("FINAL OPTIMIZED DECISIVE SCORING COMPLETED")
        print(f"{'='*100}")

def main():
    """Main function to test final optimized decisive scoring"""
    scorer = FinalOptimizedDecisiveScorer()
    ...
    try:
        logger.info("Starting FINAL OPTIMIZED DECISIVE stock scoring test...")
