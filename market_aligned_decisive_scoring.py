#!/usr/bin/env python3
"""
MARKET-ALIGNED DECISIVE Stock Scoring System
Provides realistic Buy/Sell ratings aligned with current market sentiment
Matches AI analyst recommendations and current market conditions
"""
...
class MarketAlignedDecisiveScorer:
    """Market-aligned decisive stock scorer with realistic ratings"""
    ...
    def calculate_fundamental_health(self, fundamental: Dict) -> float:
        """Calculate fundamental health score (0-100) - MARKET-ALIGNED"""
        try:
            score = 0
            factors = 0
            
            # Market Cap - Market-aligned scoring
            if fundamental.get('market_cap') and fundamental['market_cap'] > 0:
                ...
                if market_cap_b >= 100:  # Large cap
                    ...
                    score += 23  # Good growth potential
                elif market_cap_b >= 1:  # Small cap
                    score += 20  # Higher risk/reward
                else:  # Micro cap
                    score += 18  # Speculative
                factors += 1
            
            # Profitability - Market-aligned
            if fundamental.get('net_income_ttm') and fundamental['net_income_ttm'] > 0:
                ...
                score += 15   # Negative but not extreme
                factors += 1
            
            # Debt to Equity - Market-aligned
            ...
            # Return on Equity - Market-aligned
            ...
            # Market-aligned scoring - higher base scores for major companies
            if final_score >= 85:
                ...
                return 54.0  # Below average
            else:
                return 46.0  # Poor
            
        except Exception as e:
            logger.error(f"Error calculating fundamental health: {e}")
            return 60.0  # Higher default score
    
    def calculate_technical_health(self, technical: Dict, price_history: List) -> float:
        """Calculate technical health score (0-100) - MARKET-ALIGNED"""
        try:
            score = 0
            factors = 0
            
            # Price momentum - Market-aligned
            ...
            if len(price_history) > 1:
                ...
                if prev_price > 0:
                    ...
                    if momentum > 0.08:  # Strong uptrend
                        ...
                        score += 18  # Moderately bearish
                    elif momentum > -0.08:
                        score += 15  # Bearish
                    else:
                        score += 12   # Very bearish
                    factors += 1
            ...
            if technical.get('rsi') is not None:
                rsi = technical['rsi']
                if 35 <= rsi <= 65:
                    score += 32  # Neutral (good)
                elif 25 <= rsi <= 75:
                    score += 28  # Acceptable range
                elif 15 <= rsi <= 85:
                    score += 24  # Extended range
                elif 5 <= rsi <= 95:
                    score += 20  # Extreme levels
                else:
                    score += 16  # Very extreme
                factors += 1
            
            # Moving Averages - Market-aligned
            ...
            # Market-aligned technical scoring
            if final_score >= 85:
                return 90.0  # Exceptional
            elif final_score >= 75:
                return 82.0  # Excellent
            elif final_score >= 65:
                return 74.0  # Good
            elif final_score >= 55:
                return 66.0  # Above average
            elif final_score >= 45:
                return 58.0  # Average
            elif final_score >= 35:
    ...
    def calculate_vwap_sr_score(self, technical: Dict, ticker: str) -> Tuple[float, Dict]:
        """Calculate VWAP & Support/Resistance score (0-100) - MARKET-ALIGNED"""
        try:
            current_price = technical['close'] / 100.0
            ...
            # VWAP Analysis (40% weight) - Market-aligned
            vwap_score = 0
            if vwap > 0:
            ...
            # Support Analysis (30% weight) - Market-aligned
            support_score = 0
            ...
            # Resistance Analysis (30% weight) - Market-aligned
            resistance_score = 0
    ...
    def calculate_composite_score(self, scores: Dict) -> float:
        """Calculate composite score with MARKET-ALIGNED weights"""
        try:
            fundamental = scores.get('fundamental_health', 60)
            technical = scores.get('technical_health', 60)
            vwap_sr = scores.get('vwap_sr', 60)
            
            # MARKET-ALIGNED WEIGHTS
            ...
            composite = (fundamental * 0.35) + (technical * 0.25) + (vwap_sr * 0.40)
    ...
    def get_rating(self, score: float) -> str:
        """Get rating based on score - MARKET-ALIGNED THRESHOLDS"""
        if score >= 85:
            return "Strong Buy"
        elif score >= 75:
            return "Buy"
    ...
    def test_scoring_system(self, test_tickers: List[str] = None):
        """Test the market-aligned decisive scoring system"""
        if not test_tickers:
        ...
        # Display results
        print("=" * 100)
        print("MARKET-ALIGNED DECISIVE STOCK SCORING SYSTEM RESULTS - VALID DATA STOCKS ONLY")
        print("=" * 100)
        ...
        print(f"\n✅ MARKET-ALIGNED SYSTEM ASSESSMENT:")
        ...
        print(f"   • Market-aligned scoring thresholds")
        print(f"   • Heavy VWAP & S/R weight (40% of total score)")
        print(f"   • Realistic market sentiment alignment")
        
        print(f"\n{'='*100}")
        print("MARKET-ALIGNED DECISIVE SCORING COMPLETED")
        print(f"{'='*100}")

def main():
    """Main function to test market-aligned decisive scoring"""
    scorer = MarketAlignedDecisiveScorer()
    ...
    try:
        logger.info("Starting MARKET-ALIGNED DECISIVE stock scoring test...")
